...
<script setup>
// `ref` is already imported by the sibling <script> block above (Vue merges both
// script blocks into one scope), so we only import `computed` here to avoid a
// duplicate-declaration parse error.
