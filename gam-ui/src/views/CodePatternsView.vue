<template>
  <div class="h-full flex flex-col overflow-hidden">
    <PageHeader title="Code Patterns" subtitle="Quy tắc nhận diện email & trích mã theo game/platform" icon="🧩" :connected="connected" @refresh="load" />
...
const gameOptions = computed(() => [
  { value: '', label: '(không)' },
  ...games.value.map((g) => ({ value: g.name, label: g.game_name })),
])
...
async function load() {
  ...
  try {
    const [p, g] = await Promise.all([
      ...
      getList('GAM Game', { fields: ['name', 'game_name'], limit: 500, order_by: 'game_name asc' }),
    ])
    patterns.value = p
    games.value = g
  } catch {
